...
bool VescUart::processReadPacket(uint8_t * message) {
	...
	switch (packetId){
		...
		case COMM_GET_VALUES: // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
			data.temp_fet = buffer_get_float16(message, 10.0, &index);
			data.temp_motor = buffer_get_float16(message, 10.0, &index);
			...
			data.v_in 		= buffer_get_float16(message, 10.0, &index);		// 2 bytes - GET_INPUT_VOLTAGE()
			...
			data.tachometer_abs 		= buffer_get_int32(message, &index);				// 4 bytes - mc_interface_get_tachometer_abs_value(false)
			...
			/*
			data.temp_mos3 = buffer_get_float16(message, 10.0, &index);
			data.temp_mos4 = buffer_get_float16(message, 10.0, &index);
			data.temp_mos5 = buffer_get_float16(message, 10.0, &index);
			data.temp_mos6 = buffer_get_float16(message, 10.0, &index);
			data.temp_pcb = buffer_get_float16(message, 10.0, &index);
			data.current_motor = buffer_get_float32(message, 100.0, &index);
			data.current_in = buffer_get_float32(message, 100.0, &index);
			data.duty_now = buffer_get_float16(message, 1000.0, &index);
			data.v_in = buffer_get_float16(message, 10.0, &index);
			data.rpm = buffer_get_int32(message, &index);
			data.amp_hours = buffer_get_float32(message, 10000.0, &index);
			data.amp_hours_charged = buffer_get_float32(message, 10000.0, &index);
			index += 8;
			data.tachometer = buffer_get_int32(message, &index);
			data.tachometer_abs = buffer_get_int32(message, &index);
			data.fault_code = message[index++];
			*/
			return true;
...
void VescUart::setSCUBA_POWER_ON(void) {
	int32_t index = 0;
	int payloadSize = 3;
	uint8_t payload[payloadSize];
...
void VescUart::sendSCUBA_POWER_KEEPALIVE(void){
	int32_t index = 0;
	int payloadSize = 1;
	uint8_t payload[payloadSize];
...
void VescUart::setSCUBA_LED_OFF(void) {
	int32_t index = 0;
	int payloadSize = 3;
	uint8_t payload[payloadSize];
...
void VescUart::setSCUBA_LED_ON(void) {
	int32_t index = 0;
	int payloadSize = 3;
	uint8_t payload[payloadSize];
